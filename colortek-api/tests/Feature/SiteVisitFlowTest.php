...
beforeEach(function (): void { $this->seed(ReferenceSeeder::class); Storage::fake('local'); });
function siteEngineer(): User { return User::factory()->inDepartment('site')->create(); }
...
it('scenario 2: critical no forces not_ready', function (): void {
    ...
    app(TaskService::class)->claim($readiness, $engineer); app(TaskService::class)->start($readiness->fresh(), $engineer);
...
it('scenario 3: humidity warning only', function (): void {
    ...
    ['visit' => $visit, 'engineer' => $engineer] = startSiteVisitFlow(); Sanctum::actingAs($engineer);
...
it('scenario 7: forty rows two pages', function (): void {
    ['visit' => $visit, 'engineer' => $engineer] = startSiteVisitFlow(); Sanctum::actingAs($engineer);
    $rows = []; for ($i = 0; $i < 40; $i++) { $rows[] = ['page_number' => $i < 23 ? 1 : 2, 'line_number' => ($i % 23) + 1, 'element_name' => $i === 0 ? 'X' : '', 'sort_order' => $i]; }
...
it('scenario 11: override releases one task', function (): void {
    ...
    $manager = User::factory()->create(); $manager->assignRole('management'); Sanctum::actingAs($manager);
...
it('scenario 12: ready releases held tasks', function (): void {
    ...
    $siteTask = $instance->tasks()->where('task_definition_id', $siteDef->id)->sole();
    $siteTask->update(['status' => TaskStatus::Pending]); $project->update(['site_ready' => true]);
    app(\App\Services\Workflow\WorkflowEngine::class)->releaseSiteHeldTasks($project->fresh());
...
it('scenario 14: Arabic checklist labels from API', function (): void {
    $this->seed(\Database\Seeders\SiteChecklistSeeder::class); Sanctum::actingAs(siteEngineer());
