...
use App\Enums\ActivitySeverity; use App\Enums\TaskStatus; use App\Models\Project; use App\Models\Setting; use App\Models\Task; use App\Models\User;
use App\Services\Activity\ActivityRecorder; use App\Services\Audit\AuditLogger; use App\Services\Workflow\WorkflowEngine; use Illuminate\Support\Facades\DB;
final class SiteBlockService {
    ...
    public function applyBlock(Project $project): void { DB::transaction(function() use ($project): void {
    ...
    public function releaseBlock(Project $project): void { DB::transaction(function() use ($project): void {
        $project->update(['site_ready'=>true]); $this->workflowEngine->releaseSiteHeldTasks($project->fresh());
    }); }
    public function override(Task $task, User $user, string $reason): Task { return DB::transaction(function() use ($task,$user,$reason): Task {
