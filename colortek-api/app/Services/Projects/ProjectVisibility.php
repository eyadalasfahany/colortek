...
use App\Enums\TaskStatus; use App\Models\ActivityEvent; use App\Models\Project; use App\Models\User; use Illuminate\Database\Eloquent\Builder;
final class ProjectVisibility {
 public function canView(User $u, Project $p): bool {
  if ($u->can("project.view_all") || $p->sales_user_id === $u->id) return true;
 ...
 public function applyToProjects(Builder $q, User $u): Builder {
  if ($u->can("project.view_all")) return $q;
  $d=$u->departments()->pluck("departments.id");
