...
export async function getSiteVisit(id: number): Promise<SiteVisitDetail> {
  const response = await axiosInstance.get<ApiEnvelope<SiteVisitDetail>>(
    `/site-visits/${id}`,
  );
...
export async function getSiteChecklistItems(): Promise<SiteChecklistItem[]> {
  const response = await axiosInstance.get<{ data: SiteChecklistItem[] }>(
    "/site-checklist-items",
  );
  return response.data;
}

export async function patchSiteVisitDraft(
  id: number,
  payload: Record<string, unknown>,
): Promise<SiteVisitDetail> {
  const response = await axiosInstance.patch<ApiEnvelope<SiteVisitDetail>>(
    `/site-visits/${id}`,
    payload,
  );
  return unwrapData(response);
...
export async function saveSiteMeasurements(
  id: number,
...
export async function submitSiteVisit(
  ...
  const response = await axiosInstance.post<
    ApiEnvelope<SiteVisitDetail> & { meta?: { humidity_warning?: boolean } }
  >(`/site-visits/${id}/submit`, {
    answers,
    signed_attachment_id: signedAttachmentId,
    client_signatory_name: clientSignatoryName,
  });
  return {
    visit: unwrapData(response),
    humidityWarning: response.meta?.humidity_warning === true,
  };
}
