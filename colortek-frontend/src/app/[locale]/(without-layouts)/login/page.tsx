  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error instanceof ApiError) {
        setErrorMessage(error.message || t("wrongCredentials"));
        if (error.errors) {
    ...
    <div className="flex min-h-full items-center justify-center bg-background-gray-secondary_alt_2 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="mb-6 flex-col items-start gap-4">
          ...
          <p className="text-xs text-text-tertiary">{t("languageHint")}</p>
        </CardHeader>
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <TextField className="w-full flex-col gap-1.5" required>
            ...
            <InputGroup>
              ...
              <InputGroupButton
                size="icon-sm"
                className="me-1 text-text-secondary hover:text-text-primary"
